...
def extract_bronze(
    ...
    overlay_raw_path = out_dir / "overlay_raw.mp4"
    writer = None
    if write_overlay:
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        writer = cv2.VideoWriter(str(overlay_raw_path), fourcc, float(fps), (width, height))
        if not writer.isOpened():
            raise RuntimeError("Failed to open VideoWriter for overlay_raw.mp4")
    ...
    if writer is not None:
        writer.release()
        # Re-encode to H.264 for browser playback compatibility
        import subprocess
        result = subprocess.run([
            "ffmpeg", "-y",
            "-i", str(overlay_raw_path),
            "-vcodec", "libx264",
            "-crf", "23",
            "-preset", "fast",
            "-pix_fmt", "yuv420p",
        ...
        # Clean up raw file regardless of ffmpeg result
        try:
            overlay_raw_path.unlink()
        except Exception:
            pass
        # If ffmpeg failed, fall back to raw file
        if result.returncode != 0 or not overlay_path.exists():
            if overlay_raw_path.exists():
                overlay_raw_path.rename(overlay_path)
