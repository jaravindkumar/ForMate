...
#status-badge.waiting{border-color:rgba(245,158,11,.5);color:#fbbf24;}
#status-badge.active{border-color:rgba(255,63,63,.5);color:#ff6b6b;}
#status-badge.done{border-color:rgba(187,255,0,.4);color:#BBFF00;}
...
.flag.ok  {background:rgba(46,204,113,.25);border:1px solid rgba(46,204,113,.4);color:#7ee8a2;}
.flag.warn{background:rgba(245,158,11,.25);border:1px solid rgba(245,158,11,.45);color:#ffd060;}
.flag.bad {background:rgba(255,63,63,.3);border:1px solid rgba(255,63,63,.5);color:#ff8080;}
...
#btn-main.stop {background:rgba(255,63,63,.9);color:#fff;}

#btn-save{
  font-weight:700;
  font-size:.8rem;
  letter-spacing:.06em;
  border:1px solid rgba(187,255,0,.4);
  border-radius:50px;
...
#cam-off-icon{font-size:3.5rem;}
#cam-off-txt{font-size:.9rem;font-weight:600;letter-spacing:.05em;}
#cam-off-sub{font-size:.72rem;color:#1D1D28;max-width:220px;text-align:center;line-height:1.6;}
...
<div id="cam-container">
  ...
  <!-- Off state -->
  <div id="cam-off">
    <div id="cam-off-icon">&#128247;</div>
    <div id="cam-off-txt">FORMate Live</div>
    <div id="cam-off-sub">BlazePose runs in your browser. No data leaves your device.</div>
  </div>
...
function checkThresholds(kp){
  ...
  if(EXERCISE==="deadlift"){
    ...
    if(ok(11,23)){const d=Math.abs(xn(11)-xn(23));f.drift=d<.07?{st:"ok",lbl:"Bar OK"}:d<.14?{st:"warn",lbl:"Bar drifting"}:{st:"bad",lbl:"Bar too far!"};}
    if(ok(23,25)){const d=yn(25)-yn(23);f.hinge=d>.03?{st:"ok",lbl:"Hinge OK"}:{st:"warn",lbl:"Hinge deeper"};}
  } else {
    if(ok(23,25)){const c=xn(25)-xn(23);f.knee=c>=-.02?{st:"ok",lbl:"Knees OK"}:c>=-.07?{st:"warn",lbl:"Knee caving"}:{st:"bad",lbl:"Knee cave!"};}
    if(ok(23,25)){const d=yn(23)-yn(25);f.depth=d<=.03?{st:"ok",lbl:"Good depth"}:d<=.12?{st:"warn",lbl:"Go deeper"}:{st:"bad",lbl:"Too shallow"};}
    if(ok(11,23)){const l=Math.abs(xn(11)-xn(23));f.lean=l<.07?{st:"ok",lbl:"Upright OK"}:l<.14?{st:"warn",lbl:"Leaning fwd"}:{st:"bad",lbl:"Too much lean!"};}
...
function updateFlags(flags){
  document.getElementById("flags-wrap").innerHTML=
    Object.entries(flags)
      .filter(([k,{st}])=>st!=="ok")
      .map(([k,{st,lbl}])=>'<div class="flag '+st+'">'+
        (st==="bad"?"▲ ":"△ ")+lbl+"</div>").join("");
}
...
function updateRep(hipY,idx){
  hipHist.push(hipY);if(hipHist.length>90)hipHist.shift();
  if(calibN<CALIB){calibN++;if(standY===null||hipY<standY)standY=hipY;return;}
  if(botY===null){
    const s=[...hipHist].sort((a,b)=>a-b);
    standY=s[Math.floor(s.length*.15)];
    botY  =s[Math.floor(s.length*.85)];
  }
  const r=botY-standY;if(r<.03)return;
  const dT=standY+r*.35,bT=standY+r*.65,rT=standY+r*.30;

  if(repState==="IDLE"){
    if(hipY>dT){repState="DESCENDING";repStart=idx;peakY=hipY;setStatus("active");}
  }else if(repState==="DESCENDING"){
    if(hipY>peakY)peakY=hipY;
    if(hipY>=bT)repState="AT_BOTTOM";
    else if(hipY<rT&&(idx-repStart)<10)repState="IDLE";
  }else if(repState==="AT_BOTTOM"){
    if(hipY>peakY)peakY=hipY;
    if(hipY<dT)repState="ASCENDING";
  }else if(repState==="ASCENDING"){
    if(hipY>peakY){peakY=hipY;repState="AT_BOTTOM";}
    else if(hipY<=rT&&(idx-repStart)>=8){
      repCount++;
...
async function detect(){
  ...
  try{
    ...
    if(poses.length>0){
      ...
      if(lh&&rh&&lh.score>.4&&rh.score>.4)updateRep((lh.y+rh.y)/2,sessionFrames.length);
      // capture frame
...
async function toggleCamera(){
  ...
  if(!running){
    ...
    try{
      ...
      calibN=0;standY=null;botY=null;sessionFrames=[];
...
function saveSession(){
  window.parent.postMessage({
    type:"formate_session",repCount,
    exercise:EXERCISE,frames:sessionFrames,
...
""".replace("EXERCISE_PLACEHOLDER", ex_js)
    ...
    result_data = components.html(movenet_html, height=750, scrolling=False)
