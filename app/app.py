...
<style>
*{box-sizing:border-box;margin:0;padding:0;}
body{background:#06060A;font-family:Arial,sans-serif;color:#F0EEF8;overflow-x:hidden;}
.shell{display:grid;grid-template-columns:260px 1fr;gap:1.5rem;padding:1.5rem;min-height:100vh;}
@media(max-width:680px){.shell{grid-template-columns:1fr;}}
.left{display:flex;flex-direction:column;gap:.9rem;}
.logo{font-family:Arial Black,sans-serif;font-size:1.5rem;font-weight:900;}
.logo-form{color:#BBFF00;}.logo-ate{color:#F0EEF8;}
.rep-box{background:#13131A;border:1px solid #1D1D28;border-radius:14px;padding:1.1rem;text-align:center;}
.rep-num{font-size:4.5rem;line-height:1;color:#BBFF00;font-weight:900;letter-spacing:-.03em;transition:color .2s;}
.rep-lbl{font-size:.58rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#5A5870;margin-top:.2rem;}
.rep-ex{font-size:.65rem;color:#2A2838;text-transform:uppercase;letter-spacing:.1em;margin-top:.1rem;}
.status{display:inline-flex;align-items:center;gap:.45rem;padding:.3rem .85rem;border-radius:20px;font-size:.65rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;}
...
.flag{font-size:.72rem;font-weight:700;padding:.4rem .7rem;border-radius:8px;}
...
.step{display:flex;gap:.6rem;align-items:flex-start;padding:.55rem .75rem;border-radius:9px;background:#13131A;border:1px solid #1D1D28;font-size:.75rem;color:#5A5870;line-height:1.5;}
.step-n{color:#BBFF00;font-weight:900;flex-shrink:0;}
.btn{font-weight:900;font-size:.95rem;letter-spacing:.05em;border:none;border-radius:10px;padding:.8rem 1.5rem;width:100%;cursor:pointer;transition:all .18s;margin-top:.15rem;}
.btn-start{background:#BBFF00;color:#06060A;}
.btn-start:hover{background:#D4FF4D;}
.btn-stop{background:rgba(255,63,63,.15);color:#ff6b6b;border:1px solid rgba(255,63,63,.3);}
.btn-save{background:#1D1D28;color:#BBFF00;border:1px solid rgba(187,255,0,.2);}
.save-info{background:#13131A;border:1px solid #1D1D28;border-radius:10px;padding:.75rem;font-size:.75rem;color:#5A5870;text-align:center;}
.save-info b{color:#BBFF00;}
.right{display:flex;flex-direction:column;gap:.75rem;}
.cam-wrap{position:relative;background:#0D0D12;border-radius:16px;overflow:hidden;border:1px solid #1D1D28;aspect-ratio:4/3;}
video{width:100%;height:100%;object-fit:cover;display:block;transform:scaleX(-1);}
canvas{position:absolute;top:0;left:0;width:100%;height:100%;}
.cam-off{display:flex;align-items:center;justify-content:center;height:100%;min-height:280px;flex-direction:column;gap:.6rem;color:#2A2838;}
.fps-badge{position:absolute;top:.6rem;right:.6rem;background:rgba(6,6,10,.85);border:1px solid #1D1D28;border-radius:6px;padding:.2rem .55rem;font-size:.65rem;font-weight:700;color:#BBFF00;z-index:10;}
</style>
</head>
<body>
<div class="shell">
  <div class="left">
    <div>
      <div class="logo"><span class="logo-form">FORM</span><span class="logo-ate">ate</span></div>
      <div style="font-size:.55rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#5A5870;margin-top:.1rem;">Live Trainer &middot; BlazePose</div>
    </div>
    <div id="status-wrap"><div class="status off"><div class="sdot"></div><span>Camera Off</span></div></div>
    <div class="rep-box">
      <div class="rep-num" id="rep-num">0</div>
      <div class="rep-lbl">Reps Completed</div>
      <div class="rep-ex" id="rep-ex">EXERCISE_PLACEHOLDER</div>
    </div>
    <div class="flags" id="flags-wrap"></div>
    <div class="steps">
      <div class="step"><span class="step-n">01</span>Hit Start &mdash; BlazePose loads in browser.</div>
      <div class="step"><span class="step-n">02</span>Walk into position. First rep auto-detected.</div>
      <div class="step"><span class="step-n">03</span>Joints turn red/orange when form breaks live.</div>
      <div class="step"><span class="step-n">04</span>Stop &amp; Save for full AI report.</div>
    </div>
    ...
    <div class="save-info" id="save-info" style="display:none"><b id="save-count">0</b> frames &middot; <b id="save-reps">0</b> reps</div>
  </div>
  <div class="right">
    <div class="cam-wrap" id="cam-wrap">
      <div class="cam-off" id="cam-off"><div style="font-size:2.5rem">&#128247;</div><div style="font-size:.8rem">Camera will appear here</div></div>
...
const BP={L_SH:11,R_SH:12,L_EL:13,R_EL:14,L_WR:15,R_WR:16,
          L_HIP:23,R_HIP:24,L_KN:25,R_KN:26,L_AN:27,R_AN:28};
const CONNS=[[11,12],[11,13],[13,15],[12,14],[14,16],
             [11,23],[12,24],[23,24],[23,25],[25,27],[24,26],[26,28]];
const KEY_JOINTS={
  back:[11,12,23,24], drift:[11,12], hinge:[23,24],
  knee:[25,26], depth:[23,24,25,26], lean:[11,12,23,24]
};
...
// ── Thresholds — BlazePose returns normalised 0-1 x,y ────────────
function checkThresholds(kp){
  const xn=i=>kp[i]&&kp[i].score>.4?kp[i].x:null;
  const yn=i=>kp[i]&&kp[i].score>.4?kp[i].y:null;
  const ok=(...ids)=>ids.every(i=>xn(i)!==null);
  ...
  if(EXERCISE==="deadlift"){
    ...
    if(ok(11,23)){
      const d=Math.abs(xn(11)-xn(23));
      f.drift=d<.07?{st:"ok",lbl:"Bar path OK"}:d<.14?{st:"warn",lbl:"Bar drifting"}:{st:"bad",lbl:"Bar too far!"};
    }
    if(ok(23,25)){
      const d=yn(25)-yn(23);
      f.hinge=d>.03?{st:"ok",lbl:"Good hinge"}:{st:"warn",lbl:"Hinge deeper"};
    }
  } else {
    if(ok(23,25)){
      const c=xn(25)-xn(23);
    ...
    if(ok(23,25)){
      const d=yn(23)-yn(25);
      f.depth=d<=.03?{st:"ok",lbl:"Good depth"}:d<=.12?{st:"warn",lbl:"Go deeper"}:{st:"bad",lbl:"Too shallow"};
    }
    if(ok(11,23)){
      const l=Math.abs(xn(11)-xn(23));
...
// ── Draw skeleton ─────────────────────────────────────────────────
// BlazePose x,y are 0-1 normalised — scale to W,H for pixel drawing
// Video is CSS-mirrored (scaleX(-1)) so we mirror x: draw at (1-x)*W
function drawSkeleton(ctx,kp,flags,W,H){
  const px=i=>kp[i]?(1-kp[i].x)*W:0;  // mirror to match CSS scaleX(-1)
  const py=i=>kp[i]?kp[i].y*H:0;
  ...
  for(const[a,b] of CONNS){
    if(vs(a)<.3||vs(b)<.3)continue;
    ctx.globalAlpha=.88;ctx.strokeStyle=cc;
    ctx.beginPath();ctx.moveTo(px(a),py(a));ctx.lineTo(px(b),py(b));ctx.stroke();
  }
  // Joints
  ctx.globalAlpha=1;
  ...
  // Text labels for warn/bad
  let yo=32;ctx.font="bold 14px Arial,sans-serif";ctx.globalAlpha=1;
  for(const[k,{st,lbl}] of Object.entries(flags)){
    if(st==="ok")continue;
    const col=st==="bad"?"#FF4444":"#F59E0B";
    const tw=ctx.measureText(lbl).width;
    ctx.fillStyle="rgba(0,0,0,.8)";ctx.fillRect(10,yo-17,tw+18,24);
    ctx.fillStyle=col;ctx.fillText(lbl,19,yo);yo+=30;
  }
}

// ── Flag chips in sidebar ─────────────────────────────────────────
function updateFlags(flags){
  document.getElementById("flags-wrap").innerHTML=
    Object.entries(flags).map(([k,{st,lbl}])=>
      '<div class="flag '+st+'">'+(st==="ok"?"✓":"▲")+" "+lbl+"</div>"
    ).join("");
}

// ── Rep state machine (normalised hip Y) ─────────────────────────
let repCount=0, hipHistory=[], repState="IDLE";
let repStart=0, peakHipY=0;
let standingY=null, bottomY=null, calibFrames=0;
...
function updateRepState(hipY, frameIdx){
  ...
  if(calibFrames<MAX_CALIB){
    calibFrames++;
    if(standingY===null||hipY<standingY) standingY=hipY;
    return;
  }
  if(bottomY===null){
    const s=[...hipHistory].sort((a,b)=>a-b);
    standingY=s[Math.floor(s.length*.15)];
    bottomY  =s[Math.floor(s.length*.85)];
  }
  const range=bottomY-standingY;
  if(range<.03)return;
  const descentThr=standingY+range*.35;
  const bottomThr =standingY+range*.65;
  const returnThr =standingY+range*.30;

  if(repState==="IDLE"){
    if(hipY>descentThr){repState="DESCENDING";repStart=frameIdx;peakHipY=hipY;sessionStarted=true;setStatus("active");}
  } else if(repState==="DESCENDING"){
    if(hipY>peakHipY)peakHipY=hipY;
    if(hipY>=bottomThr)repState="AT_BOTTOM";
    else if(hipY<returnThr&&(frameIdx-repStart)<10)repState="IDLE";
  } else if(repState==="AT_BOTTOM"){
    if(hipY>peakHipY)peakHipY=hipY;
    if(hipY<descentThr)repState="ASCENDING";
  } else if(repState==="ASCENDING"){
    if(hipY>peakHipY){peakHipY=hipY;repState="AT_BOTTOM";}
      ...
      el.style.color="#fff";setTimeout(()=>el.style.color="#BBFF00",300);
      repState="IDLE";
    }
  }
}
...
function setStatus(s){
  const m={off:"Camera Off",waiting:"Waiting for movement...",active:"Recording",done:"Session Complete"};
  const c={off:"off",waiting:"waiting",active:"active",done:"done"};
  document.getElementById("status-wrap").innerHTML=
    '<div class="status '+(c[s]||"off")+'"><div class="sdot"></div><span>'+(m[s]||"")+"</span></div>";
}

// ── State ─────────────────────────────────────────────────────────
let detector=null, stream=null, rafId=null, running=false;
let sessionFrames=[], lastFpsTime=performance.now(), fpsCount=0;
...
async function detect(){
  ...
  // Internal canvas resolution = video resolution
  canvas.width=W; canvas.height=H;
  // Canvas CSS size must match the *displayed* video box (CSS mirrored via scaleX)
  // No extra CSS needed — canvas fills the cam-wrap via width/height:100% CSS
  ...
  try{
    ...
    if(poses.length>0){
      ...
      drawSkeleton(ctx,kp,flags,W,H);
      updateFlags(flags);
      // Hip Y for rep counter — use normalised coords directly
      const lh=kp[BP.L_HIP], rh=kp[BP.R_HIP];
      if(lh&&rh&&lh.score>.4&&rh.score>.4){
        updateRepState((lh.y+rh.y)/2, sessionFrames.length);
      }
      ...
      sessionFrames.push(tmp.toDataURL("image/jpeg",.65));
...
async function toggleCamera(){
  ...
  if(!running){
    btn.textContent="Loading BlazePose...";btn.disabled=true;
    try{
      detector=await poseDetection.createDetector(
        poseDetection.SupportedModels.BlazePose,
        {runtime:"tfjs",modelType:"lite",enableSmoothing:true}
      ...
      running=true;repCount=0;hipHistory=[];repState="IDLE";
      calibFrames=0;standingY=null;bottomY=null;
      sessionStarted=false;sessionFrames=[];
      document.getElementById("rep-num").textContent="0";
      document.getElementById("rep-ex").textContent=EXERCISE.toUpperCase();
      btn.textContent="STOP & ANALYSE";btn.className="btn btn-stop";btn.disabled=false;
      setStatus("waiting");
      detect();
    }catch(e){
    ...
    btn.textContent="START CAMERA";btn.className="btn btn-start";
    setStatus("done");
    if(sessionFrames.length>5)document.getElementById("btn-save").style.display="block";
  }
}

// ── Save → Streamlit ──────────────────────────────────────────────
function saveSession(){
  window.parent.postMessage({type:"formate_session",repCount,exercise:EXERCISE,frames:sessionFrames,frameCount:sessionFrames.length},"*");
  document.getElementById("btn-save").textContent="Sending...";
