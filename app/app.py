...
let lastSpoken={};       // key → timestamp, prevents spam
let lastFlagState={};    // key → last severity, only speaks on change
const VOICE_COOLDOWN=5000; // ms between same cue

function speak(text, priority=false){
  if(!voiceEnabled) return;
  if(!window.speechSynthesis) return;
  // Cancel current if priority (rep count)
...
function canSpeak(key){
  const now=Date.now();
  if(!lastSpoken[key]||now-lastSpoken[key]>VOICE_COOLDOWN){
    lastSpoken[key]=now;
    return true;
  }
  return false;
...
// Form cue messages per flag label
const VOICE_CUES={
  // Squat
  "Knee cave!":   "Knees out",
...
function speakFlags(flags){
  for(const[key,{st,lbl}]of Object.entries(flags)){
    if(st==="ok") continue;
    const prev=lastFlagState[key];
    // Only speak if severity got worse or it's been a while
    const worseningOrNew = !prev || (prev==="ok") ||
      (prev==="warn"&&st==="bad");
    const cue=VOICE_CUES[lbl];
    if(cue && (worseningOrNew || canSpeak("flag_"+key))){
      if(worseningOrNew) lastSpoken["flag_"+key]=Date.now();
      speak(cue);
    }
    lastFlagState[key]=st;
  }
  // Reset flags that are now ok
  for(const key of Object.keys(lastFlagState)){
    if(!flags[key]||flags[key].st==="ok") lastFlagState[key]="ok";
  }
}

function speakRepCount(n){
  // Say the number, every rep
  speak(String(n), true);
  // Milestone cues
  if(n===5)  setTimeout(()=>speak("5 reps. Good work."),400);
  if(n===10) setTimeout(()=>speak("10 reps. Excellent."),400);
}
...
async function toggleCamera(){
  ...
  if(!running){
    ...
    setStatus("done");
    speak(repCount+" reps complete. Great session.", true);
