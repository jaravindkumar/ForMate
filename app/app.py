...
body{background:#0D0D12;font-family:Archivo,sans-serif;color:#F0EEF8;}
...
.badge{font-size:.62rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;padding:.22rem .6rem;border-radius:12px;}
.badge.green{background:rgba(187,255,0,.1);border:1px solid rgba(187,255,0,.2);color:#BBFF00;}
.badge.loading{background:rgba(90,88,112,.1);border:1px solid #1D1D28;color:#5A5870;}
.flags{display:flex;gap:.35rem;flex-wrap:wrap;}
.flag{font-size:.62rem;font-weight:600;padding:.2rem .55rem;border-radius:10px;}
.flag.ok{background:rgba(46,204,113,.08);border:1px solid rgba(46,204,113,.15);color:#7ee8a2;}
...
<div class="bar">
  <span class="badge loading" id="status-badge">Loading MoveNet...</span>
...
const KP = {L_SHOULDER:5,R_SHOULDER:6,L_HIP:11,R_HIP:12,L_KNEE:13,R_KNEE:14,L_ANKLE:15,R_ANKLE:16,L_ELBOW:7,R_ELBOW:8,L_WRIST:9,R_WRIST:10};
const CONNS = [[5,6],[5,7],[6,8],[7,9],[8,10],[5,11],[6,12],[11,12],[11,13],[12,14],[13,15],[14,16]];
const KEY_JOINTS = {
...
function checkThresholds(kp,W,H){
  // Normalise to 0-1 for resolution-independent thresholds
  const x=i=>kp[i]?kp[i].x/W:0;
  const y=i=>kp[i]?kp[i].y/H:0;
  const v=i=>kp[i]?kp[i].score:0;
  const flags={};
  if(EXERCISE==="deadlift"){
    if(v(5)>.4&&v(11)>.4&&v(13)>.4){
      const a=angle2d(x(5),y(5),x(11),y(11),x(13),y(13));
      flags.back=a>=145?{st:"ok",l:"Back OK"}:a>=115?{st:"warn",l:"Back rounding"}:{st:"bad",l:"Severe back round!"};
    }
    if(v(5)>.4&&v(11)>.4){
      const d=Math.abs(x(5)-x(11));
      flags.drift=d<0.08?{st:"ok",l:"Bar path OK"}:d<0.15?{st:"warn",l:"Bar drifting"}:{st:"bad",l:"Bar too far!"};
    }
    if(v(11)>.4&&v(13)>.4){
      const d=y(13)-y(11);
      flags.hinge=d>0.04?{st:"ok",l:"Good hinge"}:{st:"warn",l:"Hinge deeper"};
    }
  } else {
    if(v(11)>.4&&v(13)>.4){
      const c=x(13)-x(11);
      flags.knee=c>=-0.02?{st:"ok",l:"Knees OK"}:c>=-0.06?{st:"warn",l:"Knee caving"}:{st:"bad",l:"Knee cave!"};
    }
    if(v(11)>.4&&v(13)>.4){
      const d=y(11)-y(13);
      flags.depth=d>=0.02?{st:"ok",l:"Good depth"}:d>=-0.05?{st:"warn",l:"Go deeper"}:{st:"bad",l:"Too shallow"};
    }
    if(v(5)>.4&&v(11)>.4){
      const l=Math.abs(x(5)-x(11));
      flags.lean=l<0.08?{st:"ok",l:"Upright OK"}:l<0.15?{st:"warn",l:"Leaning forward"}:{st:"bad",l:"Too much lean!"};
    }
  }
  return flags;
}
...
function jointColor(idx,flags){
  ...
  for(const[key,joints]of Object.entries(KEY_JOINTS)){
    ...
    if(joints.includes(idx)){
      if(flags[key].st==="bad")return"#FF3F3F";
      if(flags[key].st==="warn"&&worst==="ok")worst="warn";
    }
  }
  return worst==="warn"?"#F59E0B":"#BBFF00";
}

function drawFrame(ctx,kp,flags,W,H){
  ...
  for(const[a,b]of CONNS){
    if(!kp[a]||!kp[b]||kp[a].score<0.3||kp[b].score<0.3)continue;
    ctx.strokeStyle=cc;
    ctx.beginPath();ctx.moveTo(kp[a].x,kp[a].y);ctx.lineTo(kp[b].x,kp[b].y);ctx.stroke();
  }
  ctx.globalAlpha=1;
  for(let i=0;i<kp.length;i++){
    if(!kp[i]||kp[i].score<0.3)continue;
    ctx.beginPath();ctx.arc(kp[i].x,kp[i].y,5,0,2*Math.PI);
    ctx.fillStyle=jointColor(i,flags);ctx.fill();
    ctx.strokeStyle="#06060A";ctx.lineWidth=1.5;ctx.stroke();
  }
  // Labels
  let yo=22; ctx.font="bold 12px Archivo,sans-serif";
  for(const[k,{st,l}]of Object.entries(flags)){
    if(st==="ok")continue;
    const col=st==="bad"?"#FF5050":"#F59E0B";
    ctx.fillStyle="rgba(6,6,10,0.7)";
    ctx.fillRect(8,yo-13,ctx.measureText(l).width+14,18);
    ctx.fillStyle=col;ctx.fillText(l,14,yo);yo+=22;
  }
}

function updateFlagsUI(flags){
  const el=document.getElementById("flags");
  el.innerHTML=Object.values(flags).map(({st,l})=>
    '<span class="flag '+st+'">'+l+'</span>'
  ).join("");
}
...
async function init(){
  ...
  document.getElementById("status-badge").textContent="MoveNet Thunder Ready";
  ...
  async function loop(){
    ...
    try{
      ...
      if(poses.length>0){
        ...
        const flags=checkThresholds(kp,W,H);
        drawFrame(ctx,kp,flags,W,H);
        updateFlagsUI(flags);
      }
    }catch(e){}
...
init().catch(e=>{ document.getElementById("status-badge").textContent="MoveNet failed: "+e.message; });
</script>
...
// ── THRESHOLD CHECK ───────────────────────────────────────────────
function checkThresholds(kp, W, H) {
  // Normalise pixel coords to 0-1 for resolution-independent thresholds
  const x   = (i) => kp[i] ? kp[i].x / W : 0;
  const y   = (i) => kp[i] ? kp[i].y / H : 0;
  ...
  if (EXERCISE === "deadlift") {
    ...
    if (vis(KP.L_SHOULDER)>.4 && vis(KP.L_HIP)>.4 && vis(KP.L_KNEE)>.4) {
      const ang = angle2d(x(KP.L_SHOULDER),y(KP.L_SHOULDER),
                          x(KP.L_HIP),     y(KP.L_HIP),
                          x(KP.L_KNEE),    y(KP.L_KNEE));
    ...
    if (vis(KP.L_SHOULDER)>.4 && vis(KP.L_HIP)>.4) {
      ...
      if (drift < 0.08)       flags.drift = {st:"ok",   lbl:"Bar path OK"};
      else if (drift < 0.15)  flags.drift = {st:"warn", lbl:"Bar drifting"};
      else                    flags.drift = {st:"bad",  lbl:"Bar too far!"};
    }
    // 3. Hip hinge depth
    if (vis(KP.L_HIP)>.4 && vis(KP.L_KNEE)>.4) {
      const diff = y(KP.L_KNEE) - y(KP.L_HIP);
      flags.hinge = diff > 0.04 ? {st:"ok", lbl:"Good hinge"} : {st:"warn", lbl:"Hinge deeper"};
    }
  } else {
    // SQUAT
    // 1. Knee cave
    if (vis(KP.L_HIP)>.4 && vis(KP.L_KNEE)>.4) {
      ...
      if (cave >= -0.02)      flags.knee = {st:"ok",   lbl:"Knees OK"};
      else if (cave >= -0.06) flags.knee = {st:"warn", lbl:"Knee caving"};
      else                    flags.knee = {st:"bad",  lbl:"Knee cave!"};
    }
    // 2. Squat depth
    if (vis(KP.L_HIP)>.4 && vis(KP.L_KNEE)>.4) {
      const depth = y(KP.L_HIP) - y(KP.L_KNEE);
      if (depth >= 0.02)       flags.depth = {st:"ok",   lbl:"Good depth"};
      else if (depth >= -0.05) flags.depth = {st:"warn", lbl:"Go deeper"};
      else                     flags.depth = {st:"bad",  lbl:"Too shallow"};
    }
    // 3. Forward lean
    if (vis(KP.L_SHOULDER)>.4 && vis(KP.L_HIP)>.4) {
      const lean = Math.abs(x(KP.L_SHOULDER) - x(KP.L_HIP));
      if (lean < 0.08)       flags.lean = {st:"ok",   lbl:"Upright OK"};
      else if (lean < 0.15)  flags.lean = {st:"warn", lbl:"Leaning forward"};
...
function updateRepState(hipY, frameIdx) {
  ...
  if (range < 0.03) return; // not enough movement — normalised units
  ...
  try {
    ...
    if (poses.length > 0) {
      ...
      if (lHip && rHip && lHip.score > 0.4 && rHip.score > 0.4) {
        // Normalise to 0-1 so state machine thresholds are resolution-independent
