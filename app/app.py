...
#rep-ex{font-size:.6rem;letter-spacing:.12em;text-transform:uppercase;color:#BBFF00;opacity:.75;}
...
<div id="cam-container">
  <div id="cam-off">
    ...
    <div style="font-size:.95rem;font-weight:700;color:#5A5870">FORMate Live Trainer</div>
...
function drawSkeleton(ctx,kp,flags,W,H){
  // Front cam: CSS scaleX(-1) mirrors video, so we also flip x in canvas
  // Back cam: no CSS mirror, draw at raw pixel coords
  const mirror=facingMode==="user";
  const px=i=>kp[i]?(mirror?W-kp[i].x:kp[i].x):0;
  const py=i=>kp[i]?kp[i].y:0;
  ...
  // flag labels
  let yo=32;ctx.font="bold 15px Arial,sans-serif";ctx.globalAlpha=1;
  for(const[k,{st,lbl}]of Object.entries(flags)){
    if(st==="ok")continue;
    const col=st==="bad"?"#FF4444":"#F59E0B";
    const tw=ctx.measureText(lbl).width;
...
async function detect(){
  ...
  try{
    ...
    if(poses.length>0){
      ...
      drawSkeleton(ctx,kp,flags,W,H);
      updateFlags(flags);
      // rep counting — normalise hip Y
      const lh=kp[MV.L_HIP],rh=kp[MV.R_HIP];
      if(lh&&rh&&lh.score>.3&&rh.score>.3)
        updateRep(((lh.y+rh.y)/2)/H, sessionFrames.length);
      // save frame
      ...
      sessionFrames.push(tmp.toDataURL("image/jpeg",.7));
    }else{
      // pose not detected — show hint
      ctx.font="bold 14px Arial";ctx.fillStyle="rgba(255,255,255,.35)";
      ctx.textAlign="center";
      ctx.fillText("Point camera at full body",W/2,H-20);
      ctx.textAlign="left";
    }
  ...
  if(!running){
    ...
    try{
      ...
      stream=await navigator.mediaDevices.getUserMedia(
        {video:{facingMode:facingMode,width:{ideal:720},height:{ideal:1280}},audio:false}
      );
      const video=document.getElementById("video");
...
async function flipCamera(){
  ...
  // stop current stream
  if(stream)stream.getTracks().forEach(t=>t.stop());
  // mirror video CSS for front cam
  const video=document.getElementById("video");
  video.style.transform=facingMode==="user"?"scaleX(-1)":"scaleX(1)";
